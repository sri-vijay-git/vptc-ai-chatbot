...
pdf.set_text_color(33, 77, 130)
pdf.cell(0, 12, clean('Vignesh Polytechnic College (VPTC)'), align='C', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
pdf.set_font('Helvetica', '', 11)
pdf.set_text_color(80, 80, 80)
pdf.cell(0, 7, clean('Complete College Knowledge Base for AI Reference'), align='C', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
pdf.set_text_color(0,0,0)
pdf.ln(4)
...
kv('Location','Melputhiyandal Village, Manalurpet Road, Tiruvannamalai, Tamil Nadu - 606 603')
...
for name,desc in [
    ...
    pdf.set_font('Helvetica', 'B', 10)
    pdf.set_x(pdf.get_x()+4)
    pdf.write(5.5, clean(name)+': ')
    pdf.set_font('Helvetica', '', 10)
    pdf.multi_cell(0, 5.5, clean(desc))
pdf.ln(2)
...
for q,a in faqs:
    pdf.set_font('Helvetica', 'B', 10)
    pdf.multi_cell(0, 5.5, clean('Q: '+q))
    pdf.set_font('Helvetica', '', 10)
    pdf.set_x(pdf.get_x()+4)
    pdf.multi_cell(0, 5.5, clean('A: '+a))
    pdf.ln(1)
...
out = r'backend\data\documents\VPTC_Complete_KnowledgeBase.pdf'
pdf.output(out)
